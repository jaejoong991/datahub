...
import { ReturnStatus, NegotiationStatus, SellerProofStatus, SellerCompensationStatus, ReturnSolution, } from "../../schemas/returns.js";
...
describe("ReturnsManager", () => {
    ...
    describe("getReturnDisputeReason", () => {
        it("should get available dispute reasons", async () => {
            ...
            expect(mockShopeeFetch).toHaveBeenCalledWith(mockConfig, "/returns/get_return_dispute_reason", {
                method: "GET",
                auth: true,
                params: {
                    return_sn: "200203171852695",
    ...
    describe("getReverseTrackingInfo", () => {
        it("should get reverse tracking info for seller validation", async () => {
            ...
            expect(mockShopeeFetch).toHaveBeenCalledWith(mockConfig, "/returns/get_reverse_tracking_info", {
                method: "GET",
                auth: true,
                params: {
                    return_sn: "2206150VT13E3MQ",
            ...
            expect(result.response.tracking_info[0].tracking_description).toBe("Parcel has been picked up by our logistics partner");
        });
    ...
    describe("Schema Evaluation Check", () => {
        it("should successfully reference all exported enums and constants", () => {
            ...
            expect(NegotiationStatus.PENDING_RESPOND).toBe("PENDING_RESPOND");
            expect(SellerProofStatus.PENDING).toBe("PENDING");
            expect(SellerCompensationStatus.PENDING_REQUEST).toBe("PENDING_REQUEST");
            expect(ReturnSolution.REFUND_ONLY).toBe("REFUND_ONLY");
        });
    });
});
//# sourceMappingURL=returns.manager.test.js.map